...
def locale_change(bot, update, user, args):
    ...
    if len(args) > 0:
        if args[0] in locale_list:
            ...
            return octeon.message("Your language now is <code>%s</code>" % args[0], parse_mode="HTML")
        else:
            return octeon.message(text="Unknown language: <code>%s</code>" % args[0], parse_mode='HTML')
    else:
        return octeon.message(text="Locales:\n" + "\n".join(locale_list) + "\nUse <code>/language codename</code> to change language", parse_mode="HTML")
